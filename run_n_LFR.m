%Tests Noisy Case (first layer random communities and others informative)
%LFR graphs
%The function tests each methods on 10 SBM graphs with 4 communities of size
%125, p=0.1 probability arc intercommunity (informative layers), s=0.1 probability arc inter and intra community (noise layers)
%each method is tested 10 times on the same graph (because in the noisy case the communities are reindexed randomly). 
%It prints the average results in a csv file called "data.csv"
%INPUT: q probability arc intracommunity (informartive layers)
%       k1 number of informative layers
%       k2 number of noisy layers 

function run_n_LFR(k1,k2)

Methods={'Time';'Acc';'nmi'};

%p=0.1; %probability arc intercommunity (informartive layers)
%s=0.1; %probability arc inter and intra community (noise layers)
...
c=[];
for i=1:m
   c = cat(2,c,i*ones(1,32));
end
k = k1+k2; %total number of layers
...
%MAtrix 
oldFolder = cd;
cd ./LFR/dataset_binary_network_01
data_file = ['network_noisy_' num2str(k) '_' num2str(N_iter2) '.mat'];
load(data_file);
cd(oldFolder)
for i=1:k
    A{i}=M(:,:,i);
...
%Run the methods on the martrix
%Generalized Louvain
for N_iter = 1:10
            tx1 = tic;
...
for N_iter = 1:10
    tx19 = tic;
    [label19] = spectral_centroid_multiview_onkernel(A, k, m, lambda2);
    tEnd19 = toc(tx19);
...
%for N_iter = 1:10
    ...
    Time24 = [Time24 tEnd24];
    Acc24 = [Acc24 (n-wrong(c,label24))/(n)];
...
T=table(Methods,CA,CVP1,CVP3,CVP5,CVP7,CVP9,MA_r2,MA_r3,MVP12,MVP13,MVP32,MVP33,MVP52,MVP53,MVP72,MVP73,MVP92,MVP93,CoRegm,AWPm,MCGCm,PM,MT,INFOMAP,SCML,PMMm);
file_name = ['Artificial_results_noise_' num2str(k1) '_' num2str(k2) '.csv'];
writetable(T,file_name) %csv file
end