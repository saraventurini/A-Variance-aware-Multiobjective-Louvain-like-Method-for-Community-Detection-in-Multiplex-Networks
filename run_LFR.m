...
data_file = ['network_info_' num2str(k) '_' num2str(N_iter2) '.mat'];
load(data_file);
cd(oldFolder)

%M=adjacent_matrix_generator_multi(L,p,q,k);
for i=1:k
    A{i}=M(:,:,i);
end
...
%Run the methods on the MAtrix
    ...
    layers = B(:,2);
    labels = zeros(k,n); %check if each layer same partition %labels(s,:) labels s-layer
...
%average values
GL=[mean(Time1);mean(Acc1);mean(nmi1)];
...
MT=[mean(Time23);mean(Acc23);mean(nmi23)];
INFOMAP =[mean(Time24);mean(Acc24);mean(nmi24)];
SCML =[mean(Time25);mean(Acc25);mean(nmi25)];
PMMm =[mean(Time26);mean(Acc26);mean(nmi26)];

T=table(Methods,GL,CVM1,CVM3,CVM5,CVM7,CVM9,MA2,MA3,MVM12,MVM13,MVM32,MVM33,MVM52,MVM53,MVM72,MVM73,MVM92,MVM93,CoRegm,AWPm,MCGCm,PM,MT,INFOMAP,SCML,PMMm);
file_name = ['Artificial_results_info_' num2str(k) '.csv'];
