...
%
function [COMTY, ending] = EVM(M,lambda,z)
if nargin < 1
  ...
  for i=1:N
    ...
    for j=1:length(NB) %consider each neighbor of node i
      ...
      if (G(Cj) == 0) %if we haven't already considereted the community of node j
        ...
        G(Cj) = (1-lambda)*M_DQ - lambda* GV_j; 
    ...
    %Recalculate
    Ck = (COM==Cnew); %insert node i in the new community
    for s=1:k
...
if (z == 1) 
  ...
  while 1
    ...
    [COMt, e] = EVM(Mnew,lambda,0); %apply the first phase on this reduced network
    if (e ~= 1)
      COMfull = sparse(1,N);
      COMcur = COMt.COM{1};
      for p=1:Ncom
