%Louvain Multiobjective Method Function F+
% Inputs : 
% M : t3 dim tensor, M(:,:,s)=adjacent matrix layer s 
% h : length of the filter
% lambda : weight of variance in function
...
%   L{5}=Average  (average)
%   L{6}=Function  (funtion)
%


function [L, ending] = MVP(M,lambda,h,z)

if nargin < 1
  error('not enough argument');
end
if nargin < 2
  error('not lambda defined');
end
if nargin < 3
  error('not h defined');
end
...
%average
Average = sum(Q)/k;
Q_minus_avg = Q-Average;
%variance
if k==1
   Variance = 0; 
else 
   Variance = (sum(Q_minus_avg.^2)) / (k-1); %variance
end
%function
Function = (1-lambda) * Average + lambda * Variance;

%If no edges in any layer or just one node
if sum(m>0)==0 || N==1
  ending = 0;
  L={[Q,{COM},SumTot,SumIn,Average,Function]};
else
...
while (GAIN == 1) %filter has changed 
  ...
  R = randperm(N);
  for g=1:N 
      i = R(g); %for each node, in this order
      ...
      for l=1:length(L_o) %for each partition in the filter
          ...
          [L_new,U] = step_1 (L_o{l}{1},L_o{l}(2),L_o{l}{3},L_o{l}{4},L_o{l}{5},L_o{l}{6},k,NBc,K,M,N,m,L_new,lambda,i);
  ...
  if length(L_old)~=length(L_new)
      ...
      if size(C,1)*size(C,2) == sum(sum(C)) %no filter changes
          GAIN=0;
      end 
  end
  Niter = Niter + 1;
...
if (z==1)
      ...
      Mnew=M; %new weight matrix of this iteration
...
%Compute step_1 
function [L,U] = step_1(Q,COM,SumTot,SumIn,Average,Function,k,NBc,K,M,N,m,L,lambda,i)
...
for j=1:length(NB) %consider each neighbor j of node i
    ...
    %variance
    M_DQ = sum(DQ_vec)/k; %gain average
    Q_minus_avg = Q-Average;
    if k==1
       GV_j = 0;
    else
        tmp = DQ_vec - M_DQ;
        GV_j = ((sum(tmp.^(2))) / (k-1)) + (2/(k-1)) * sum( sum(Q_minus_avg .* tmp) ); %gain variance
    end 
    %gain of the function
    GF_j = (1-lambda)*M_DQ + lambda* GV_j;
    if GF_j > -10^(-14) %if positive gain (function increases)
        %Recalculate
        F_j = Function + GF_j; %function
        ...
        A_j = Average + M_DQ; %average
        %add the partition to the filter
        [L] = add_to_filter(Q_j,COM,SumTot3,SumIn3,A_j,F_j,L,k);
        U=1;           
    end
    end
end
...
function [L] = add_to_filter(QQ,COM,SumTot,SumIn,Average,Function,L,k)
...
if  ~DD %the new point is not dominated  
  ...
  L(empties) = [];
  L{end+1}=[QQ,COM,SumTot,SumIn,Average,Function];  %add the new point to the filter
   
end
end
else %empty filter
    L={[QQ,COM,SumTot,SumIn,Average,Function]};  %I add the new point to the filter
end
end
...
%function of every element of the filter 
if length(L)>h 
   Function=cell(1,length(L));
   for l=1:length(L)
        %function
        Function{l} = L{l}{6};
   end  
...
for t=1:(length(L)-h)
  [mn]=min([Function{:}]);
  v=find([Function{:}]==mn); %if more element with same average values, delate the last one 
                             %(therefore with h=1 I have same result as Louvain Expansion Method Function F+)
  idx_t=v(end);
  Function{idx_t}=Inf;
  L{idx_t}=[];
end
...
%Re-index community partition randomly (line 120)
