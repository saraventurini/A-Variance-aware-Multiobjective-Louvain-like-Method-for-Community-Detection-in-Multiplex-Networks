...
% 
function [COMTY, ending] = GL_s(M,z)
if nargin < 1
...
while (gain == 1)  %no increase of average possible moving one node 
  gain = 0;
  %Index communitites by size (function reindex_com line 249)
  for i=1:N
...
if (z == 1) 
  ...
  while 1
    ...
    [COMt, e] = GL_s(Mnew,0); %apply the first phase on this reduced network 
    if (e ~= 1)
      COMfull = sparse(1,N);
      COMcur = COMt.COM{1};
      for p=1:Ncom
