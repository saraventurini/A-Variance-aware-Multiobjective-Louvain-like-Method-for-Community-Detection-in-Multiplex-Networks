%Louvain Expansion Method F+
%communities reindex randomly
% Inputs : 
...
%
function [COMTY, ending] = EVP(M,lambda,z)
...
Function = (1-lambda) * Average + lambda * Variance; %function
...
gain = 1;
while (gain == 1) %no increase of function possible moving one node
  gain = 0;
  %Randomized labels
  R = randperm(N);
  for g=1:N
    ...
    for j=1:length(NB) %consider each neighbor of node i
      ...
      if (G(Cj) == 0) %if we haven't already considereted the community of node j
        ...
        M_DQ = sum(DQ_vec)/k; %Average gain
        if k==1
            GV_j = 0; %gain variance
        else 
            tmp = DQ_vec - M_DQ;
            GV_j = ((sum(tmp.^(2))) / (k-1)) + (2/(k-1)) * sum( sum(Q_minus_avg .* tmp) ); %gain variance
        end 
        %gain of the function
        G(Cj) = (1-lambda)*M_DQ + lambda* GV_j; %Function gain 
        if G(Cj) > best_increase %if positive gain (function increases)
          best_increase = G(Cj); %gain function
          Q_t = DQ_vec; %gain of modularity;
          M_t = M_DQ; %gain average
          V_t = GV_j;%gain of variance
    ...
    if best_increase > -10^(-15) %if best increase is positive
      ...
      Q = Q + Q_t; %modularity
      Average = Average + M_t;
      Variance = Variance + V_t;
      Function = Function + best_increase; %function
      Q_minus_avg = Q-Average;
    end
    %Recalculate
    Ck = (COM==Cnew);
...
COMTY.Variance(1) = Variance;
COMTY.Function(1) = Function;
COMTY.Niter(1) = Niter;
...
if (z == 1)  
  ...
  while 1
    ...
    [COMt, e] = EVP(Mnew,lambda,0); %apply the first phase on this reduced network
    if (e ~= 1)
      COMfull = sparse(1,N);
      COMcur = COMt.COM{1};
      for p=1:Ncom
      ...
      COMTY.Variance(j) = COMt.Variance(1);
      COMTY.Function(j) = COMt.Function(1);
...
%Re-index community partition randomly (line 103)
